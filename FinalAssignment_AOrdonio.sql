...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'ITFnd130FinalDB_AOrdonio')
	 Begin 
	  Alter Database [ITFnd130FinalDB_AOrdonio] set Single_user With Rollback Immediate;
	  Drop Database ITFnd130FinalDB_AOrdonio;
	 End
	Create Database ITFnd130FinalDB_AOrdonio;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use ITFnd130FinalDB_AOrdonio;
...
--(I accidentally locked myself out of viewing my stuff, so I coded this)
Revoke all on Courses to PUBLIC;
Revoke all on vCourses to Private;

Revoke all on Students to PUBLIC;
Revoke all on vStudents to Private;

Revoke all on CourseEnrollment to Public;
Revoke all on vCourseEnrollment to Private;
go
